...
Проект не использует `fetch`, поэтому обычно работает и при прямом открытии файла (`file://`).
Для стабильной проверки интерактива и якорной навигации рекомендуется запускать через статический сервер.
...
- Форма: валидация, inline ошибки, `aria-live`, имитация отправки, disabled-кнопка, toast.
