...
const CONFIG = {
  ...
  formEndpoint: 'https://formsubmit.co/ajax/sashatanin777@gmail.com',
  swipeThreshold: 48,
  toastVisibleMs: 2600,
...
const localeUi = {
  ru: {
    ...
    formSendError: 'Не удалось отправить сообщение. Попробуйте позже.',
  ...
  en: {
    ...
    formSendError: 'Unable to send your message. Please try again later.',
    formSending: 'Sending...',
...
function initForm() {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const payload = {
      ...
      _subject:
        state.locale === 'en'
          ? 'New request from Nordline Studio website'
          : 'Новая заявка с сайта Nordline Studio',
      _captcha: 'false',
      _template: 'table',
    ...
    try {
      ...
      const ok = response.ok && (!result || result.success !== 'false');
      ...
      status.textContent = locale.formSuccessStatus;
      status.className = 'form-status success';
      showToast(locale.formSuccessToast);
    } catch (error) {
      console.error(error);
      status.textContent = locale.formSendError;
      status.className = 'form-status error';
    } finally {
      if (submitBtn instanceof HTMLButtonElement) {
